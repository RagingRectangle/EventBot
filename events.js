...
async function cronUpdates() {
    ...
    const autoMessages = JSON.parse(fs.readFileSync('./autoUpdates.json'));

    for (const [firstMsgId, channelId] of Object.entries(autoMessages)) {
        try {
            ...
            const botMessages = (await channel.messages.fetch({
                ...
                .filter(msg => msg.author.id === client.user.id && msg.embeds.length > 0); // Only consider messages with embeds

            // Delete all fetched bot messages
            await Promise.all(botMessages.map(msg => msg.delete()));

            const eventComponent = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setLabel(config.buttonLabel).setCustomId(`eventBot~refresh`).setStyle(ButtonStyle.Primary)
            );
            if (config.emojiID) {
                eventComponent.components[0].setEmoji(config.emojiID);
            }
            const componentsWithButton = [eventComponent];
            const componentsWithoutButton = [];

            const newMessages = [];
            for (let i = 0; i < eventEmbeds.length; i++) {
                const currentComponents = i === eventEmbeds.length - 1 ? componentsWithButton : componentsWithoutButton;
                const sentMessage = await channel.send({
                    embeds: [eventEmbeds[i]],
                    components: currentComponents
                }).catch(console.error);
                if (sentMessage) {
                    newMessages.push(sentMessage.id);
                }
                await new Promise(resolve => setTimeout(resolve, 1000)); // Small delay
            }

            // Update autoUpdates.json with only the last message ID
            if (config.autoUpdate && newMessages.length > 0) {
                const updatedAutoMessages = {};
                updatedAutoMessages[newMessages[newMessages.length - 1]] = channelId;
                fs.writeFileSync('./autoUpdates.json', JSON.stringify(updatedAutoMessages));
            }

        } catch (err) {
            console.log(`Error updating messages in channel ${channelId}: ${err}`);
        }
    }
} // End of cronUpdates()
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.customId === 'eventBot~refresh') {
        ...
        try {
            const eventEmbeds = await createEmbeds();
            if (eventEmbeds.length === 0) {
                return await interaction.editReply('No events to display at the moment.');
            }
            ...
            if (config.autoUpdate && newMessages.length > 0) {
                const autoMessages = {};
                autoMessages[newMessages[newMessages.length - 1]] = channel.id;
                fs.writeFileSync('./autoUpdates.json', JSON.stringify(autoMessages));
            }

            await interaction.deleteReply().catch(console.error); // Remove the "is thinking..." message

        } catch (error) {
            console.error(error);
            await interaction.editReply('An error occurred while refreshing events.');
        }
    }
}); // End of button interaction handler
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.commandName === 'events') {
        try {
            ...
            // Update autoUpdates.json with only the last message ID
            if (config.autoUpdate && sentMessages.length > 0) {
                const autoMessages = {};
                autoMessages[sentMessages[sentMessages.length - 1]] = interaction.channel.id;
                fs.writeFileSync('./autoUpdates.json', JSON.stringify(autoMessages));
            }
